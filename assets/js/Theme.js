var Theme = {
    ...
    miscFunctions: function($){
        if($('.slicknav_nav').length > 0){
            var htop = $('.header .bottom_layer').offset().top + 80.76;
            $('.slicknav_nav').css('top', htop + "px");
        }

        if($('body.in-mobile').length > 0){
            $('#desktop6, #desktop6 .slick-slide').css('height', ($(window).height() - $('#desktop6').offset().top) + 'px');
        }
    },

    smoothScroll: function($){
        $('a[href^="#"]:not(a[href="#request-quote"])').click(function(e) {
            e.preventDefault();
            ...
            if ($target.length) {
                ...
                if (targetOffset >= $(window).scrollTop() && targetOffset < ($(window).scrollTop() + $(window).height())) {
                    // Target element is already in view, no need to scroll
                    return;
                }
            }
    ...
    modalQuote: function($){
        $(document).ready(function() {
            $('a[href="#request-quote"]').fancybox({
              type: 'inline',
              beforeLoad: function() {
                // Ensure that the inline content is visible when the modal is opened
    ...
    worksCarousel: function($) {
        function adjustCarousel() {
            var windowWidth = $(window).width();
            var slidesToShow = (windowWidth <= 450) ? 1 : 3;
    
            $('.slick-carousel').slick('slickSetOption', 'slidesToShow', slidesToShow);
        }
